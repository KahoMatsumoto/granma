...
async function handleEvent(event) {
	...
	await ref_user.onse('value').then(async function(snapshot) {
        ...
        dialog_node:"node_1_1539094629105"}
			...
			await ref_mw.onse('value').then(async function(ss) {
...
async function docomo(text) {
	...
	await ref_docomo.onse('value').then(async function(snapshot) {
